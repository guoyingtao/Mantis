...
**Rationale**: A labeled tuple provides lightweight, zero-boilerplate named access to crop output data (translation, rotation, scale, crop size, image view size, crop region, skew degrees, sublayer transform, scroll state, and image container frame) without requiring an initializer, stored-property declarations, or a separate type definition. Callers can positionally destructure the result in a single `let` binding, which kept the original 7-field API concise. The six skew-related fields (`horizontalSkewDegrees`, `verticalSkewDegrees`, `skewSublayerTransform`, `scrollContentOffset`, `scrollBoundsSize`, `imageContainerFrame`) were appended at the end during the perspective-correction feature work, preserving backward compatibility for existing positional destructuring patterns. The intended migration path is to convert `CropInfo` to a struct when a future change requires protocol conformance or when the field count makes the tuple unwieldy.

**Consequences**: New fields appended at the end preserve existing positional destructuring, but inserting or reordering fields is a source-breaking change. Because Swift tuples cannot conform to protocols, `CropInfo` cannot directly adopt `Codable`, `Equatable`, or `Hashable`; a struct conversion would be required for those features. The 13-field width is at the practical limit for tuple readability — further growth should trigger the struct migration.
